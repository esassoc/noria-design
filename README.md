...
`theme-noria.css` filename itself):

| Placeholder | Meaning | Example |
|---|---|---|
| `Noria` | Display name (titles, brand chrome, prose) | `Beacon` |
| `noria` | Lowercase id (`data-theme`, theme filename, DocsShell `theme` prop) | `beacon` |
| `noria-design` | Repo/dir name + GitHub Pages base | `beacon-design` |
| `noria` | npm scope; package name becomes `@noria/design`; raw token prefix `--noria-*` | `beacon` |
| `N` | Short text mark for the sidebar | `B` |
| `<link href="https://fonts.googleapis.com/css2?family=Besley:ital,wght@0,400..900;1,400..900&family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />` | Where the brand's Google Fonts `<link>` tags go | `<link href="…DM+Sans…" rel="stylesheet" />` |
| `Design system and prototypes for Noria — the Deschutes basin water-banking platform` | One-line description for the landing hero | `The brand language, patterns, and prototypes for the Beacon platform` |
...
1. Copy this directory to a new sibling of `../ecology`, named `noria-design`
...
3. Rename `src/styles/theme-noria.css` → `theme-<slug>.css`.
4. Find-and-replace every placeholder from the legend above across all files.
5. Insert the brand's Google Fonts `<link>` tags at `<link href="https://fonts.googleapis.com/css2?family=Besley:ital,wght@0,400..900;1,400..900&family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />` in
...
├─ astro.config.mjs        base = /noria-design/ in prod
├─ tsconfig.json
├─ .gitignore
...
└─ src/
   ...
   ├─ styles/
   │  └─ theme-noria.css   the brand theme skeleton
   └─ pages/
