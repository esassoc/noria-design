import{i as n,b as i,a as l}from"./lit-element.C8p3bJxG.js";class b extends n{constructor(){super(),this.onKeydown=(a,e)=>{let t=null;switch(a.key){case"ArrowRight":t=this.findNextEnabledTab(e,1);break;case"ArrowLeft":t=this.findNextEnabledTab(e,-1);break;case"Home":t=this.findNextEnabledTab(-1,1);break;case"End":t=this.findNextEnabledTab(this.tabs.length,-1);break;default:return}t!==null&&(a.preventDefault(),this.selectTab(t),a.target.parentElement?.children[t]?.focus())},this.tabs=[],this.activeIndex=0,this.size="md",this.variant="underline",this.appearance="underline"}static{this.properties={tabs:{type:Array},activeIndex:{type:Number,attribute:"active-index"},size:{type:String,reflect:!0},variant:{type:String,reflect:!0},appearance:{type:String,reflect:!0}}}selectTab(a){this.tabs[a]?.disabled||(this.activeIndex=a,this.dispatchEvent(new CustomEvent("tabchange",{detail:{index:a},bubbles:!0,composed:!0})))}findNextEnabledTab(a,e){let t=a+e;for(;t>=0&&t<this.tabs.length;){if(!this.tabs[t].disabled)return t;t+=e}return null}render(){return i`
      <div class="layout">
        <div class="tabs" part="tabs" role="tablist">
          ${this.tabs.map((a,e)=>{const t=this.activeIndex===e;return i`<button
  ...
  `}}customElements.get("esa-tab-layout")||customElements.define("esa-tab-layout",b);const d=[{label:"Overview"},{label:"Activity"},{label:"Settings"}];["t1","t2","t2a"].forEach(r=>{const a=document.getElementById(r);a&&(a.tabs=d)});["t6","t3","t7","t4"].forEach(r=>{const a=document.getElementById(r);a&&(a.tabs=[{label:"One"},{label:"Two"}])});const s=document.getElementById("t5");s&&(s.tabs=[{label:"Inbox",badge:4},{label:"Drafts",disabled:!0},{label:"Archive",badge:128}]);
